...
import { FileText, Clock, Tag, CheckCircle2 } from "lucide-react"; // Import a checkmark icon

export default function TestCard({ test, hasTaken }) {
  // Receive the new 'hasTaken' prop
  return (
    <div
      className={`bg-white p-6 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 border flex flex-col relative ${
        hasTaken ? "border-green-400" : "border-slate-100"
      }`}
    >
      {/* THIS IS THE NEW BADGE: It displays if hasTaken is true */}
      {hasTaken && (
        <div className='absolute top-4 right-4 flex items-center gap-1 text-xs font-bold text-green-700 bg-green-100 px-2 py-1 rounded-full'>
      ...
      <div className='mt-auto'>
        {/* The button text now changes if the test has been taken */}
        <Link
          href={`/mock-tests/${test.id}`}
          className={`block w-full text-center px-4 py-3 text-white rounded-lg font-semibold transition-all duration-200 shadow-md hover:shadow-lg ${
            hasTaken
              ? "bg-green-600 hover:bg-green-700"
              : "bg-indigo-600 hover:bg-indigo-700"
          }`}
        >
          {hasTaken ? "View Details & Retake" : "View Details & Start"}
        </Link>
      </div>
    </div>
  );
